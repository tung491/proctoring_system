...
def print_eye_pos(img, left, right, name):
    global suspect_times
    if left == right and left != 0 and left is not None:
        if left == 1:
            text = 'Looking left'
            print(text)
            suspect_times += 1
...
while (True):
    ...
    for rect in rects:
        ...
        print_eye_pos(img, eyeball_pos_left, eyeball_pos_right, name)
